...
- (AFHTTPRequestOperation *)HTTPRequestOperationWithRequest:(NSURLRequest *)request
    ...
    void (^retryBlock)(AFHTTPRequestOperation *, NSError *) = ^(AFHTTPRequestOperation *operation, NSError *error) {
        ...
        if (retryCount > 0) {
//            NSLog(@"\nAutoRetry: Request failed: %@ %@, retry %d begining...", request.URL ,error.localizedDescription,retryCount);
...
//                NSLog(@"\nAutoRetry: Delaying retry for %d seconds...", intervalInSeconds);
...
//            NSLog(@"\nAutoRetry: Request failed:%@ %@, no more retries allowed! executing supplied failure block...",request.URL, error.localizedDescription);
...
//        NSLog(@"\nSuccess %@ : running success block...",operation.request.URL);
